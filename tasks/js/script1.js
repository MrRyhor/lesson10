...
function createRandomArr(qntElements) {
    const randomArr = new Array(qntElements).fill().map(() => getRandomNumbers())
    return randomArr
}
...
let pricesGreaterThanPreviousValue = []
for (let i = 1; i <= stockPricesList.length; i++) {
    if (stockPricesList[i] > stockPricesList[i - 1])
        pricesGreaterThanPreviousValue.push(stockPricesList[i])
}
...
const pricesPercentOfMaxPrice = stockPricesList.map(element => Math.round((element * 100) / maxPrice))
...
const sumOfPricesGreaterThan1000 = pricesGreaterThan1000.reduce((prevPrice, element) => element > 1000 ? prevPrice + element : prevPrice, 0)
...
const firstPriceGreaterThan1000 = stockPricesList.find(element => element > 1000)
...
const lasttPriceGreaterThan1000 = stockPricesList.findLast(element => element > 1000)
...
// или
const indexOfLastPriceGreaterThan1000 = stockPricesList.findLastIndex(element => element > 1000)
